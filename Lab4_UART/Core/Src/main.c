...
int main(void)
{
	...
	// Setting PB10 (TX) and PB11 (RX) to enable
	GPIOB->AFR[1] |= (4 << 8);
	GPIOB->AFR[1] |= (4 << 12);
	...
	GPIOC->OTYPER &= ~(1 << 13); // Configure the pins for the push-pull output type, where 0 represents the reset state for output push-pull.
	GPIOC->OTYPER &= ~(1 << 12);
	GPIOC->OTYPER &= ~(1 << 18);
	GPIOC->OTYPER &= ~(1 << 19);
	GPIOC->OTYPER &= ~(1 << 14);
	...
	GPIOC->OSPEEDR &= ~(1 << 12); // Set pins for low-speed operation where x0 indicates the low-speed setting.
	GPIOC->OSPEEDR &= ~(1 << 18);
	GPIOC->OSPEEDR &= ~(1 << 14);
	GPIOC->OSPEEDR &= ~(1 << 16);

	GPIOC->PUPDR &= ~(1 << 13); // Set to no pull-up/down where 00 represents neither pull-up nor pull-down.
	GPIOC->PUPDR &= ~(1 << 12);
	GPIOC->PUPDR &= ~(1 << 18);
	GPIOC->PUPDR &= ~(1 << 19);
	GPIOC->PUPDR &= ~(1 << 14);
